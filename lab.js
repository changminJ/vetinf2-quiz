...
{id:2, part:'핵산구조', topic:'Chaotropic salt 틀린문장 고치기', origin:'jokbo', type:'essay',
 q:`다음은 강의안 'Chaotropic Salt' 설명을 변형한 글이다. **틀린 부분 4곳**을 찾아 옳게 고치시오. (2025 기말 16번)

"① **공유 결합**은 DNA·RNA·단백질 같은 고분자의 2차 구조 형성과 분자의 **소수성**에 큰 영향을 미친다. ② 일반 조건에서 핵산은 수화물 껍질(물 분자)로 둘러싸여 DNA의 용해도를 유지한다. ③ 그러나 핵산에 **트로픽** 염을 첨가하면, 물 분자 간 질서 정연한 구조가 파괴되며 **친수성** 환경이 조성된다. ④ 이로 인해 핵산의 2차 구조가 풀려 실리카 표면에 쉽게 결합한다."`,
 choices:[], answer:[],
 explain:`**틀린 4곳 (강의안 기준 정정)**

1. "**공유 결합**" → **수소 결합** (고분자 2차구조·수용성에 영향 주는 건 수소결합)
2. "분자의 **소수성**" → **수용성**
3. "**트로픽** 염" → **카이오트로픽(chaotropic) 염**
4. "**친수성** 환경" → **소수성** 환경 (chaotropic salt가 물 질서 깨 → 소수성 환경 → silica 결합)

✅ **건드리면 안 되는 것(맞는 표현)**: "**2차 구조**(단백질 포함)", "**용해도 유지**", "핵산 2차구조가 **풀린다**", "실리카 결합" — 전부 강의안 그대로 옳음.

**핵심**: 함정 = 수소결합↔다른결합 / 수용성↔소수성 / 카이오트로픽↔트로픽 / 소수성↔친수성. 구조(2차)·용해도 표현은 정상. (강의안 'Chaotropic Salt' 슬라이드)`,
 exam:'2025-기말', hl2025:true, freq:1, source:'jokbo'},

{id:3, part:'RNA추출', topic:'Spin column 원리·buffer', origin:'summary', type:'summary',
 explain:`## Spin column 핵산 추출 — 흡착·세척·용출\n\n**정의**: molecular biology에서 spin column method는 **silica membrane에 핵산을 흡착시켜 정제·분리**하는 기술. 4단계 흐름:\n\n**Lyse(용해) → Bind(흡착) → Wash(세척) → Elute(용출)**\n\n**핵심 조건(시험 단골 ↑/↓)**\n| 단계 | salt | 원리 |\n|---|---|---|\n| Bind(흡착) | **High salt↑**, low temp↓ | chaotropic salt의 Na⁺가 **cation bridge**(양이온 다리)로 silica(-O⁻)와 핵산 인산(-O⁻P)을 연결 |\n| Wash(세척) | — | **80% EtOH**로 단백질·염 등 불순물 제거 (핵산은 붙은 채 유지) |\n| Elute(용출) | **Low salt↓** (주로 DW) | cation bridge 해제 → 핵산이 물에 다시 녹아 떨어짐 |\n\n## 🌉 Cation bridge(양이온 다리)란? — 강의안 5주차 슬라이드\n- silica 표면(silanol **-O⁻**)과 핵산 인산골격(**-O⁻-P**)은 **둘 다 음전하** → 그냥 두면 서로 밀어냄(반발).\n- high salt(chaotropic) 속 **Na⁺(양이온)**가 그 사이에 끼어 두 음전하를 **연결·중화** → 핵산이 silica에 흡착. 이 \"양이온 다리\"가 **cation bridge**.\n- **low salt(용출)** 되면 Na⁺가 사라져 다리가 끊김 → 핵산이 떨어져 나옴.\n- 📖 근거: 강의안 5주차 'Column-based (Silica column)' 슬라이드 (그림에 Cation Bridge 표시, Bind: salt·ethanol / Elute: low salt).\n\n**Buffer 3종 역할**\n- **Binding buffer**: high salt → DNA–silica hybrid 형성 유도\n- **Washing buffer**: 80% EtOH(수화막 제거)·Tris-HCl(pH 완충, DNA 변성 방지)·NaCl(흡착 해리 방지)\n- **Elution buffer**: low salt(DW) → 흡착 해제·용출\n\n**공회전(빈 원심)**: 마지막 wash 뒤 buffer 없이 한 번 더 원심 → **잔류 EtOH로 membrane 건조**. 이유: **잔류 ethanol이 하류 반응(PCR)을 방해**하기 때문.\n\n**한 줄 요약**: 높은 염↑·낮은 온도에서 붙이고(cation bridge), EtOH로 씻고, 낮은 염↓ 물로 떼어낸다. 공회전=ethanol 말리기.`,
 exam:'', freq:2},
...
/* ===================== PCR ===================== */
{id:16, part:'PCR', topic:'PCR thermocycle·RT-PCR 50℃ 구간', origin:'summary', type:'summary',
 graph:`<svg viewBox="0 0 600 290" style="width:100%;max-width:600px;height:auto;display:block" xmlns="http://www.w3.org/2000/svg"><rect x="0" y="0" width="600" height="290" rx="10" fill="rgba(128,128,128,0.05)"/><rect x="60" y="32" width="155" height="218" fill="rgba(255,161,22,0.16)"/><text x="137" y="270" font-size="12" font-weight="800" fill="#d97706" text-anchor="middle" font-family="sans-serif">① RT-PCR만! 50℃ 역전사</text><g stroke="#9999" stroke-width="1" stroke-dasharray="4 4"><line x1="60" y1="50" x2="575" y2="50"/><line x1="60" y1="120" x2="575" y2="120"/><line x1="60" y1="195" x2="575" y2="195"/></g><g font-size="12" fill="#8a8a8a" font-family="monospace" text-anchor="end"><text x="55" y="54">94℃</text><text x="55" y="124">72℃</text><text x="55" y="199">50℃</text></g><line x1="60" y1="32" x2="60" y2="250" stroke="#888" stroke-width="1.5"/><line x1="60" y1="250" x2="575" y2="250" stroke="#888" stroke-width="1.5"/><text x="44" y="26" font-size="12" fill="#8a8a8a" font-family="sans-serif">온도</text><text x="573" y="244" font-size="12" fill="#8a8a8a" text-anchor="end" font-family="sans-serif">시간 →</text><polyline fill="none" stroke="#1DB954" stroke-width="3" stroke-linejoin="round" stroke-linecap="round" points="60,195 215,195 250,50 330,50 365,178 430,178 465,120 560,120"/><circle cx="137" cy="180" r="12" fill="#1DB954"/><text x="137" y="184" font-size="13.5" font-weight="800" fill="#fff" text-anchor="middle" font-family="sans-serif">①</text><circle cx="290" cy="35" r="12" fill="#1DB954"/><text x="290" y="39" font-size="13.5" font-weight="800" fill="#fff" text-anchor="middle" font-family="sans-serif">②</text><circle cx="397" cy="196" r="12" fill="#1DB954"/><text x="397" y="200" font-size="13.5" font-weight="800" fill="#fff" text-anchor="middle" font-family="sans-serif">③</text><circle cx="512" cy="105" r="12" fill="#1DB954"/><text x="512" y="109" font-size="13.5" font-weight="800" fill="#fff" text-anchor="middle" font-family="sans-serif">④</text></svg>`,
 explain:`## PCR 일반 과정 (변성·결합·신장)\nPCR은 DNA polymerase로 목표 DNA를 **지수적으로 증폭**한다. 한 cycle = 3단계, 보통 30~40 cycle 반복.\n| 번호 | 단계 | 온도 | 하는 일 |\n|---|---|---|---|\n| ② | **Denaturation 변성** | **94~98℃** | ds-DNA를 단일가닥으로 분리 |\n| ③ | **Annealing 결합** | **50~65℃** (Tm−2~3℃) | primer가 template에 결합 |\n| ④ | **Extension 신장** | **72℃** (Taq 최적) | DNA polymerase가 dNTP 붙여 합성 |\n\n## ⭐ RT-PCR로 넘어가면 — 맨 앞에 50℃ 구간이 추가!\nRNA를 다루는 RT-PCR은 PCR thermocycle **전에** ① **역전사(Reverse transcription)** 단계가 붙는다.\n- 🔴 **① 50℃(40~60℃) 부근에서 길게 머무는 구간** = reverse transcriptase가 **RNA → cDNA**를 합성하는 단계.\n- 🔴 이 **50℃ 평평한 앞구간이 RT-PCR만의 특징** — 일반 PCR엔 없다! (그래프 맨 앞 주황 띠)\n- 이후 pre-denaturation(94℃) → 변성·결합·신장 cycle 반복.\n\n## 📈 온도-시간 그래프 보는 법 (가로=시간, 세로=온도)\n- **① 50℃** 긴 평평 = 역전사(RNA→cDNA), RT-PCR 전용\n- **② 94℃** 꼭대기 = 변성(ds→ss)\n- **③ 50~65℃** 골짜기 = 결합(primer annealing)\n- **④ 72℃** 중간 = 신장(DNA 합성, polymerase 활발)\n\n## 한 줄 요약\nRT-PCR = **① 50℃ 역전사(RNA→cDNA)** 먼저 → ② 94℃ 변성 → ③ 50~65℃ 결합 → ④ 72℃ 신장 반복. **앞의 50℃ 구간이 RT-PCR 핵심.**`,
 exam:'', freq:2},
...
{id:42, part:'핵산구조', topic:'Phosphodiester bond·핵산 1차구조', origin:'jokbo', type:'short',
 q:`핵산(DNA/RNA)에서 nucleotide를 잇는 **phosphodiester bond**란 무엇이며, 이 결합이 만드는 구조(1차구조)와 방향성을 쓰시오.`,
 choices:[], answer:[],
 explain:`**phosphodiester bond (인산이에스터 결합)**
- 한 nucleotide의 **3'-OH**와 다음 nucleotide의 **5'-인산(phosphate)**이 **이중 ester 결합**으로 연결되는 것.
- 이 결합이 반복되어 **당(deoxyribose)-인산 backbone**을 이루며, 이것이 핵산의 **1차 구조**다.
...
핵심: **1)역전사 → 2)RNase H로 RNA 제거 → 3)PCR**. (강의안 'RT-PCR' 3단계 슬라이드)`,
 exam:'2025-기말', hl2025:true, freq:1, source:'jokbo'},
{id:46, part:'RNA추출', topic:'Spin column 전체 protocol 영어 순서', origin:'jokbo', type:'essay',
 q:`Spin column을 이용한 RNA 추출의 **전체 lab 순서를 영어로** 순서대로 쓰시오. (2025 기말 — 강의안 영문 protocol 1~8 + 1~8 배열형. 시험은 섞어 제시 후 순서를 고르게 함)`,
 choices:[], answer:[],
 explain:`**강의안 영문 protocol 전체 순서 (Lysis·Binding 1~8 → Washing·Elution 1~8)**\n\n## ▶ Part 1. Lysis & Binding\n1. Transfer 150 µl of sample (plasma/serum/urine/culture supernatant/tissue) to a 1.5 ml tube\n2. Add 300 µl **Lysis Buffer**\n3. Vortex for 15 sec\n4. Incubate at **room temperature (RT) for 10 min**\n5. Add 300 µl **Binding Buffer** and mix well by gently vortexing\n6. Place a **spin column** in a 2 ml collection tube\n7. **Load lysate (750 µl)** on the column and **centrifuge at 13,000 rpm for 1 min** (← 핵산 흡착)\n8. Discard the flow-through and re-place the column in the same collection tube\n\n## ▶ Part 2. Washing & Elution\n1. Add 500 µl **Washing Buffer A**, centrifuge 13,000 rpm 1 min\n2. Discard flow-through, re-place column\n3. Add 500 µl **Washing Buffer B**, centrifuge 13,000 rpm 1 min\n4. Discard flow-through, re-place column, **centrifuge 13,000 rpm 1 min to dry the membrane** (residual ethanol interferes with downstream reactions) ← 🔴 공회전\n5. Place the column in an **RNase-free 1.5 ml tube**; add 40 µl **Elution Buffer** directly onto the membrane (do not touch the membrane with the tip)\n6. Incubate at RT for 1 min, then centrifuge 13,000 rpm 1 min\n7. Use the eluate (~50 µl) as **template for PCR/RT-PCR**\n\n**암기 키**: Lysis→vortex→RT 10min→Binding→load·spin(흡착)→discard / Wash A→Wash B→**dry spin(공회전)**→Elution→spin→template.\n🔴 흡착=Binding+load·spin(Part1 5~7), 해제=Elution(Part2 5). 공회전=Part2 step4(ethanol 건조). (강의안 RNA Extraction Protocol 슬라이드)`,
 exam:'2025-기말', hl2025:true, freq:1, source:'jokbo'},
{id:47, part:'RNA추출', topic:'영어 단계에서 cation bridge 생성~해제 구간', origin:'jokbo', type:'multi',
 q:`다음은 spin column RNA 추출 protocol의 **영어 단계**다. 이 중 **cation bridge가 생성되어 핵산이 silica에 결합했다가 해제될 때까지 — cation bridge가 관여하는 모든 단계**를 고르시오. (2025 기말 — 영어 선지에서 cation bridge 생성~해제 구간 고르기)`,
 choices:['Add 300 µl Lysis Buffer and vortex for 15 sec','Add 300 µl Binding Buffer and mix well by gently vortexing','Load lysate (750 µl) on the column and centrifuge at 13,000 rpm for 1 min','Add 500 µl Washing Buffer A and centrifuge at 13,000 rpm for 1 min','Re-place the column and centrifuge at 13,000 rpm for 1 min to dry the membrane','Add 40 µl Elution Buffer onto the membrane, incubate 1 min, and centrifuge'],
 answer:[2,3,4,5,6],
 explain:`**정답: ②③④⑤⑥ (① Lysis만 제외)**\n\ncation bridge는 **high salt에서 생성 → 유지 → low salt에서 해제**까지 작동한다. 이 전 구간이 정답:\n\n- ① **Lysis Buffer·vortex** = 세포 용해(lysis), 아직 cation bridge **생성 전** → ❌ 제외.\n- ② **Binding Buffer(high salt)** = Na⁺ cation bridge **생성 시작** 🟢\n- ③ **Load·centrifuge** = 핵산이 silica에 **흡착(결합 완성)** 🟢\n- ④ **Washing A/B** = 흡착 **유지**(불순물만 제거) 🟡\n- ⑤ **Dry membrane(공회전)** = 흡착 **유지**, ethanol 건조 🟡\n- ⑥ **Elution Buffer(low salt)** = cation bridge **해제 → 용출** 🔴\n\n즉 **생성(②③) → 유지(④⑤) → 해제(⑥)** 전 구간이 cation bridge 관여 단계. (강의안 RNA Extraction Protocol)`,
 exam:'2025-기말', hl2025:true, freq:1, source:'jokbo'},
{id:48, part:'RNA추출', topic:'Lysis·Silica 흡착 시점 찾기', origin:'jokbo', type:'essay',
 q:`(아래 영어 protocol을 **자료**로 두고) RNA 추출 과정에서 ① **lysis(용해)가 일어나는 단계**와 ② **핵산이 silica membrane에 흡착되는 단계**가 각각 언제인지 쓰시오.\n\n※ 시험에서는 Phase 1의 **5~7번(Binding Buffer 첨가 → column에 load → centrifuge)이 한 문장**으로 묶여 제시됨.\n\n**[자료] Phase 1**\n1. sample 150 µl를 1.5 ml tube로 옮김\n2. Add 300 µl **Lysis Buffer**\n3. Vortex 15 sec\n4. Incubate at RT 10 min\n5~7. Add 300 µl **Binding Buffer** → load lysate (750 µl) on the column → **centrifuge 13,000 rpm 1 min**\n8. discard flow-through, re-place column`,
 choices:[], answer:[],
 explain:`**① Lysis(용해)가 일어나는 시점 = Phase 1의 2~4번**\n- **Lysis Buffer 첨가(2) → vortex(3) → RT 10분 incubation(4)** 구간.\n- 여기서 세포·바이러스 막이 파괴되어 핵산이 방출되고 RNase가 억제된다.\n\n**② Silica 흡착이 일어나는 시점 = Phase 1의 5~7번 (한 문장)**\n- **Binding Buffer(high salt) 첨가 → column에 load → centrifuge** 단계.\n- high salt 조건에서 **Na⁺ cation bridge**가 형성되며 핵산이 **silica membrane에 흡착**.\n\n**핵심 구분**\n| 시점 | 단계 | 무슨 일 |\n|---|---|---|\n| Lysis | 2~4 (Lysis Buffer·vortex·RT 10min) | 세포 파괴·핵산 방출 |\n| Silica 흡착 | 5~7 (Binding Buffer·load·centrifuge) | cation bridge로 silica에 결합 |\n\n🔴 함정: **Lysis Buffer = 용해 / Binding Buffer = 흡착**. 서로 다른 buffer·다른 시점. 흡착은 **Binding Buffer 넣고 load·원심하는 5~7**부터. (강의안 RNA Extraction Protocol)`,
 exam:'2025-기말', hl2025:true, freq:1, source:'jokbo'},
{id:49, part:'RNA추출', topic:'영어 protocol 16단계 순서 배열', origin:'jokbo', type:'essay',
 q:`다음 ①~⑯은 spin column RNA 추출 protocol을 **섞어 놓은 영어 단계**다. **올바른 실험 순서대로 번호를 나열하시오.** (2025 기말 — 영어 순서 배열형)\n\n① Incubate at RT for 10 min\n② Add 500 µl Washing Buffer A and centrifuge 13,000 rpm 1 min\n③ Transfer 150 µl sample to a 1.5 ml tube\n④ Centrifuge 13,000 rpm 1 min to dry the membrane (remove residual ethanol)\n⑤ Load lysate (750 µl) on the column and centrifuge 13,000 rpm 1 min\n⑥ Use the eluate (~50 µl) as template for PCR/RT-PCR\n⑦ Vortex for 15 sec\n⑧ Add 500 µl Washing Buffer B and centrifuge 13,000 rpm 1 min\n⑨ Add 300 µl Binding Buffer and mix well by gently vortexing\n⑩ Place the column in an RNase-free tube and add 40 µl Elution Buffer onto the membrane\n⑪ Add 300 µl Lysis Buffer\n⑫ Discard the flow-through and re-place the column (after loading)\n⑬ Discard the flow-through and re-place the column (after Washing A)\n⑭ Place a spin column in a 2 ml collection tube\n⑮ Incubate at RT for 1 min, then centrifuge 13,000 rpm 1 min\n⑯ Discard the flow-through and re-place the column (after Washing B)`,
 choices:[], answer:[],
 explain:`**정답 순서:** ③ → ⑪ → ⑦ → ① → ⑨ → ⑭ → ⑤ → ⑫ → ② → ⑬ → ⑧ → ⑯ → ④ → ⑩ → ⑮ → ⑥\n\n**올바른 protocol (1~16)**\n1. (③) Transfer 150 µl sample to a 1.5 ml tube\n2. (⑪) Add 300 µl Lysis Buffer\n3. (⑦) Vortex for 15 sec\n4. (①) Incubate at RT for 10 min\n5. (⑨) Add 300 µl Binding Buffer and mix well by gently vortexing\n6. (⑭) Place a spin column in a 2 ml collection tube\n7. (⑤) Load lysate (750 µl) on the column and centrifuge 13,000 rpm 1 min\n8. (⑫) Discard the flow-through and re-place the column (after loading)\n9. (②) Add 500 µl Washing Buffer A and centrifuge 13,000 rpm 1 min\n10. (⑬) Discard the flow-through and re-place the column (after Washing A)\n11. (⑧) Add 500 µl Washing Buffer B and centrifuge 13,000 rpm 1 min\n12. (⑯) Discard the flow-through and re-place the column (after Washing B)\n13. (④) Centrifuge 13,000 rpm 1 min to dry the membrane (remove residual ethanol)\n14. (⑩) Place the column in an RNase-free tube and add 40 µl Elution Buffer onto the membrane\n15. (⑮) Incubate at RT for 1 min, then centrifuge 13,000 rpm 1 min\n16. (⑥) Use the eluate (~50 µl) as template for PCR/RT-PCR\n\n🔴 핵심 흐름: **시료→Lysis→vortex→RT10min(용해)** → **Binding→spin column→load·spin→discard(흡착)** → **Wash A→discard→Wash B→discard→dry spin(공회전)** → **Elution→incubate·spin→template(용출)**.\n흡착=Binding~load·spin / 공회전=dry membrane / 해제=Elution. (강의안 RNA Extraction Protocol)`,
 exam:'2025-기말', hl2025:true, freq:1, source:'jokbo'}
];
